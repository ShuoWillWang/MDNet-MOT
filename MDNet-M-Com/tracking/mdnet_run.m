...
det = det(det(:,5) > 0 ,:);
iL = det(det(:,1)==1 & det(:,7) > Conf_SCORE ,3:6);

for i0 = 1:size(iL,1) - 1
    if(iL(i0,3) == 0 || iL(i0,4) == 0)
        continue;
    end
    for j0 = 2:size(iL,1)
        if(iL(j0,3) == 0 || iL(j0,4) == 0)
            continue;
        end
        full = (full_over(iL(i0,:), iL(j0, :)) == 1);
        if(full)
            if(iL(i0,3) > iL(j0,3))
                iL(i0,3) = 0;
                iL(i0,4) = 0;
...
for To = 2:min(nFrames)
    ...
    detLoc = det(det(:,1)==To & det(:,7) > Conf_SCORE ,3:6);
    spf = tic;
    %% Estimation
    for m = 1:M
        ...
        if target_score{m} > SCORE
            ...
            % bbox regression
            if(opts.bbreg && target_score{m}>SCORE)
        ...
        if(target_score{m} >= SCORE)
            ...
            if(target_score{m} > 0.1 && target_score{m} <= SCORE)
        ...
        if(target_score{m} < SCORE && Out == 0)% suppress the neg score temporary
            if(target_score{m} > 0.1)
                justifi(m) = 1;
                trans_f(m) = min(1.0, 1.02*trans_f(m));
            elseif(target_score{m} <= 0.1 && target_score{m} >= -SCORE)
                justifi(m) = 0;
                trans_f(m) = min(1.0, 1.02*trans_f(m));
            else
                justifi(m) = 0;
        ...
        elseif(target_score{m} <= 0.1 && Out > 1)
            justifi(m) = 0;
            justifiF(m) = 0;
        elseif (target_score{m} <= 0.1 && Out <= 1)
            if(justifi(m) ~= 0)
    ...
    for m=1:M-1
        ...
        for n=m+1:M
            ...
            if  rat > thr/2&&(( sim > thS && target_score{m}>0.1 && target_score{n}>0.1)|| ovBool(m,n))
                    ...
                    for i=1:length(ind)
                        ...
                        if display
                            ...
                            for c = 1:M
                                ...
                                rectangle('Position', result(c,frI,:), 'EdgeColor', colormap(c,:), 'Linewidth', 3);
                                text(result(c,frI,1) + result(c,frI,3)/2,result(c,frI,2) + 12,num2str(c),'Color',colormap(c,:), 'FontSize', 18, 'HorizontalAlignment', 'center'); 
                            end
    ...
    for m=1:M
        ...
        if (target_score{m} < 0.1 && sum(ovBool(m,:)==0))
             if (occBool{m}==true)
%                 5=2;
                 occBool{m} = false;
                 occCount{m}=0;
...
%                5=5;
        ...
        if(target_score{m} > SCORE && sum(ovBool(m,:))==0)
        ...
        if((mod(To,opts.update_interval)==0 || (target_score{m}<SCORE||sum(ovBool(m,:))~=0)) && To~=nFrames)
            
            if (target_score{m}<SCORE||sum(ovBool(m,:))~=0) % short-term update
    ...
    if(mod(To,10) == 0 && ~isempty(decRes))
        if(M > 20)
            ...
            decRes = decRes(decRestemp(:,1) >= max(len * 3.5 / 7, 1),:);
        end
        if(~isempty(decRes) && To ~= nFrames)
            ...
            for idx = 1: size(decRes,1)
                ...
                stat1 = imi <= 0 || jmi <= 0 || ima <= 0 || jma <= 0;
                stat2 = abs(imi - size(img,2)) <= 0 || abs(ima - size(img,2)) <= 0 || abs(jmi - size(img,1)) <= 0 || abs(jma - size(img,1)) <= 0;
                if(stat1 || stat2)
                    continue;
                end
                situation = 0;
...
for m = 1:M
    if(Ini_Frames(m) ~= 1)
        ...
        justifi(m) = 1;
        start = Ini_Frames(m);
        img = imread(images{start});
        if(size(img,3)==1), img = cat(3,img,img,img); end
        temp_bbox = result(m, start, :);
        temp_bbox = reshape(temp_bbox, [], 4);
        % bbox train
        pos_examples = gen_samples('uniform_aspect', temp_bbox, opts.bbreg_nSamples*10, opts, 0.3, 10);
        r = overlap_ratio(pos_examples,temp_bbox);
        pos_examples = pos_examples(r > opts.posThr_init,:);
        pos_examples = pos_examples(randsample(end,min(opts.bbreg_nSamples,end)),:);
        feat_conv = mdnet_features_convX(net_conv, img, pos_examples, opts);
        X = permute(gather(feat_conv),[4,3,1,2]);
        X = X(:,:);
        bbox = pos_examples;
        bbox_gt = repmat(temp_bbox,size(pos_examples,1),1);
        bbox_reg{m} = train_bbox_regressor(X, bbox, bbox_gt);
        % draw positive/negative samples
        pos_examples = gen_samples('gaussian', temp_bbox, opts.nPos_init*2, opts, 0.1, 5);
        r = overlap_ratio(pos_examples,temp_bbox);
        pos_examples = pos_examples(r>opts.posThr_init,:);
        pos_examples = pos_examples(randsample(end,min(opts.nPos_init,end)),:);

        neg_examples = [gen_samples('uniform', temp_bbox, opts.nNeg_init, opts, 1, 10);...
                                gen_samples('whole', temp_bbox, opts.nNeg_init, opts)];
        r = overlap_ratio(neg_examples,temp_bbox);
        neg_examples = neg_examples(r<opts.negThr_init,:);
        neg_examples = neg_examples(randsample(end,min(opts.nNeg_init,end)),:);
        ...
        neg_examples = gen_samples('uniform', temp_bbox, opts.nNeg_update*2, opts, 2, 5);
        r = overlap_ratio(neg_examples,temp_bbox);
        neg_examples = neg_examples(r<opts.negThr_init,:);
        neg_examples = neg_examples(randsample(end,min(opts.nNeg_update,end)),:);
        ...
        feat_conv = mdnet_features_convX(net_conv, img, examples, opts);
        total_pos_data_Re{1,1,1,m,start} = feat_conv(:,:,:,pos_idx);
        total_neg_data_Re{1,1,1,m,start} = feat_conv(:,:,:,neg_idx);
                
        success_frames{m} = [start];
        trans_f(m) = opts.trans_f;
        scale_f(m) = opts.scale_f;
        targetLoc(m,:) = temp_bbox;
        
        %% Display
        if display
            hc = get(gca, 'Children'); delete(hc(1:end-1));
            set(hd,'cdata',img); hold on;
            rectangle('Position', result(m,start,:), 'EdgeColor', colormap(m,:), 'Linewidth', 3);
            text(result(m,start,1) + result(m,start,3)/2,result(m,start,2) + 12,num2str(m),'Color',colormap(m,:), 'FontSize', 18, 'HorizontalAlignment', 'center'); 
        
            %%rectangle('Position', result(To,:), 'EdgeColor', [1 0 0], 'Linewidth', 3);
            set(gca,'position',[0 0 1 1]);
        
            text(10,10,num2str(start),'Color','y', 'HorizontalAlignment', 'left', 'FontWeight','bold', 'FontSize', 30); 
            hold off;
            mkdir([pathSave  'branch']);
            mkdir([pathSave  'branch' '/' num2str(m)]);
            imwrite(frame2im(getframe(gcf)), [pathSave  'branch' '/' num2str(m) '/' num2str(start) '_B' num2str(m) '.jpg']);
            drawnow;
        end
        ...
        for ToR = start - 1:-1:1
            ...
            img = imread(images{ToR});
            if(size(img,3)==1), img = cat(3,img,img,img); end
    
            detLoc = det(det(:,1)==ToR,3:6);
            spf = tic;
            %% Estimation
            r = overlap_ratio(detLoc,result(m,ToR + 1,:));
            ...
            if target_score > SCORE
                ...
                % bbox regression
                if(opts.bbreg && target_score>SCORE)
                    X_ = permute(gather(feat_conv(:,:,:,idx(1:5))),[4,3,1,2]);
                    X_ = X_(:,:);
                    bbox_ = samples(idx(1:5),:);
                    pred_boxes = predict_bbox_regressor(bbox_reg{m}.model, X_, bbox_);
                    result(m,ToR,:) = round(mean(pred_boxes,1));
                end
            end
            ...
            if(target_score > SCORE)
                ...
                if(target_score > 0.1 && target_score <= SCORE)
            ...
            if(state3) Out = Out + 1; end
            if(state4) Out = Out + 1; end
            if(target_score <= SCORE && Out == 0)% suppress the neg score temporary
                if(target_score > 0.1)
                    justifi(m) = 1;
                    trans_f(m) = min(1.0, 1.02*trans_f(m));
                elseif(target_score <= 0.1 && target_score >= -SCORE)
                    justifi(m) = 0;
                    trans_f(m) = min(1.0, 1.02*trans_f(m));
                else
                    justifi(m) = 0;
            ...
            elseif(target_score <= SCORE && Out >= 2)
                justifi(m) = 0;
                justifiF(m) = 0;
            elseif (target_score <= SCORE && Out == 1)
                if(justifi(m) ~= 0)
                    justifi(m) = 0;
                    justifiF(m) = 0;
                else % consider the case searching the occlusion situation
            ...
            if(target_score>SCORE)
                ...
                feat_conv = mdnet_features_convX(net_conv, img, examples, opts);
                total_pos_data_Re{1,1,1,m,ToR} = feat_conv(:,:,:,pos_idx);
                total_neg_data_Re{1,1,1,m,ToR} = feat_conv(:,:,:,neg_idx);

                success_frames{m} = [success_frames{m}, ToR];
                if(numel(success_frames{m})>opts.nFrames_long)
                    total_pos_data_Re{1,1,1,m,success_frames{m}(end-opts.nFrames_long)} = single([]);
                end
                if(numel(success_frames{m})>opts.nFrames_short)
                    total_neg_data_Re{1,1,1,m,success_frames{m}(end-opts.nFrames_short)} = single([]);
                end
            else
                total_pos_data_Re{1,1,1,m,ToR} = single([]);
                total_neg_data_Re{1,1,1,m,ToR} = single([]);
            end

            %% Network update

            if((mod(ToR,opts.update_interval)==0 || target_score<SCORE))
                if (target_score < SCORE) % short-term update
                    temp_data = cell(1,1,1,min(opts.nFrames_short, size(success_frames{m},2)));
                    temp_data(1,1,:,:) = total_pos_data_Re(1,1,1,m,success_frames{m}(max(1,end-opts.nFrames_short+1):end));
                    pos_data = cell2mat(temp_data);
                else % long-term update
                    temp_data = cell(1,1,1,min(opts.nFrames_long, size(success_frames{m},2)));
                    temp_data(1,1,:,:) = total_pos_data_Re(1,1,1,m,success_frames{m}(max(1,end-opts.nFrames_long+1):end));
                    pos_data = cell2mat(temp_data);
                end
                temp_data = cell(1,1,1,min(opts.nFrames_short, size(success_frames{m},2)));
                temp_data(1,1,:,:) = total_neg_data_Re(1,1,1,m,success_frames{m}(max(1,end-opts.nFrames_short+1):end));
                neg_data = cell2mat(temp_data);
            ...
            if display
                ...
                rectangle('Position', result(m,ToR,:), 'EdgeColor', colormap(m,:), 'Linewidth', 3);
        
                set(gca,'position',[0 0 1 1]);
                
                if(Jus(m,ToR) == 0 || JusF(m,ToR) == 0)
                    continue;
                end
        
                text(result(m,ToR,1) + result(m,ToR,3)/2,result(m,ToR,2) + 12,num2str(m),'Color',colormap(m,:), 'FontSize', 18, 'HorizontalAlignment', 'center'); 
                text(10,10,num2str(ToR),'Color','y', 'HorizontalAlignment', 'left', 'FontWeight','bold', 'FontSize', 30); 
                hold off;
                mkdir([pathSave  'branch']);
                mkdir([pathSave  'branch' '/' num2str(m)]);
            
                imwrite(frame2im(getframe(gcf)), [pathSave  'branch' '/' num2str(m) '/' num2str(ToR) '_B' num2str(m) '.jpg']);
...
%for To = 2:min(nFrames)
    ...
    %if display
        ...
        %for m = 1:M
            ...
            %end
            %rectangle('Position', result(m,To,:), 'EdgeColor', colormap(m,:), 'Linewidth', 3);
            %text(result(m,To,1) + result(m,To,3)/2,result(m,To,2) + 12,num2str(m),'Color',colormap(m,:), 'FontSize', 18, 'HorizontalAlignment', 'center'); 
        %end
